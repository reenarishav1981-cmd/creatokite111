  ...
  return (
    <div style={{...S,background:'var(--bg)',minHeight:'100vh',overflowX:'hidden'}}>
      ...
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:1000,height:60,
        ...
        <span style={{fontFamily:'var(--fd)',fontWeight:800,fontSize:16,color:scrolled?'#ffffff':'var(--t1)',transition:'color .3s'}}>Creatokite</span>
          
        </div>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <button onClick={()=>nav('/login')} style={{background:'none',border:'none',color:scrolled?'#ffffff':'var(--t1)',cursor:'pointer',padding:'7px 14px',borderRadius:7,fontSize:13,fontWeight:600,letterSpacing:'0.3px',opacity:scrolled?1:0.92,transition:'all .3s'}} onMouseEnter={e=>{e.target.style.color='var(--acc)';e.target.style.opacity=1;}} onMouseLeave={e=>{e.target.style.color='#ffffff';e.target.style.opacity=scrolled?1:0.92;}}>Sign In</button>
